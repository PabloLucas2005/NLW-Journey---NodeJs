...
export async function createTrip(app: FastifyInstance){
  app.withTypeProvider<ZodTypeProvider>().post('/trips', {
    ...
    const message = await mail.sendMail({
      ...
      html: `
      <div>
        <p style="font-family: sans-serif;font-size: 16px; line-height: 1.6;">Você solicitou a criação de uma viagem para <strong>${destination}, Brasil</strong> nas datas de <strong>${formattedStartDate}</strong> até <strong> ${formattedEndDate} </strong> </p>
        <p></p>
